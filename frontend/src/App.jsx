...
- La **sobriété** : tu évites les clichés de la culture New Age (âme, fréquences vibratoires, haute dimension, amour et lumière)
...
export default function App() {
  ...
  const getSystemPrompt = () => {
    if (dynamicInstructions) {
      return BASE_SYSTEM_PROMPT + "\n\n## Directives supplémentaires (prioritaires) :\n" + dynamicInstructions;
    }
    return BASE_SYSTEM_PROMPT;
  };
  ...
  const sendMessage = async (text) => {
    ...
    if (adminMatch) {
      ...
      dynamicInstructions = dynamicInstructions
        ? dynamicInstructions + "\n- " + newInstruction
        : "- " + newInstruction;
      setAdminNotice("✦ Directive enregistrée : NOVA a été recentrée.");
      return;
    }
    ...
    try {
      const response = await fetch("https://nova-agent-production-8bcc.up.railway.app/api/chat", {
        method: "POST",
  ...
  return (
    <div style={styles.root}>
      ...
      <div style={styles.container}>
        ...
        {adminNotice && (
          <div style={styles.adminNotice}>{adminNotice}</div>
        )}
...
const styles = {
  root: {
    ...
    color: "#f0e8d8", overflow: "hidden", position: "relative",
  },
  ...
  videoIframe: {
    ...
    minHeight: "100vh", minWidth: "177.77vh", border: "none",
  },
  videoOverlay: {
  ...
  container: {
    ...
    alignItems: "center", padding: "40px 24px 24px", boxSizing: "border-box",
  },
  ...
  logoInner: {
    ...
    background: "radial-gradient(circle, rgba(139,90,200,0.3) 0%, rgba(200,160,80,0.15) 100%)",
    border: "1px solid rgba(200,160,80,0.3)",
    display: "flex", alignItems: "center", justifyContent: "center",
  },
  logoSymbol: { fontSize: 16, color: "#d4a84b", letterSpacing: 2 },
  title: {
    fontFamily: "'Cinzel', serif", fontSize: 48, fontWeight: 400, letterSpacing: 16,
    color: "#d4a84b", margin: "0 0 8px", textShadow: "0 0 40px rgba(200,160,80,0.6)",
  },
  ...
  adminNotice: {
    background: "rgba(200,160,80,0.15)", border: "1px solid rgba(200,160,80,0.4)",
    borderRadius: 12, padding: "10px 20px", color: "#d4a84b",
  ...
  suggestion: {
    background: "rgba(200,160,80,0.1)", border: "1px solid rgba(200,160,80,0.35)",
    borderRadius: 24, padding: "10px 18px", color: "#e8d8b8",
  ...
  userText: {
    background: "rgba(139,90,200,0.25)", border: "1px solid rgba(139,90,200,0.4)",
    borderRadius: "20px 20px 4px 20px", padding: "14px 18px",
    fontSize: 15, lineHeight: 1.7, maxWidth: "80%", color: "#f0e8d8",
  },
  aiText: {
    background: "rgba(255,255,255,0.08)", border: "1px solid rgba(200,160,80,0.25)",
    borderRadius: "4px 20px 20px 20px", padding: "16px 20px",
    fontSize: 15, lineHeight: 1.9, maxWidth: "90%", color: "#ede0cc",
  },
  dots: { display: "flex", gap: 6, alignItems: "center", height: 20 },
  inputArea: { width: "100%", paddingTop: 20 },
  inputWrap: {
    display: "flex", alignItems: "flex-end", gap: 12,
    background: "rgba(255,255,255,0.1)", border: "1px solid rgba(200,160,80,0.35)",
    borderRadius: 16, padding: "12px 16px",
  },
  textarea: {
    flex: 1, background: "transparent", border: "none", outline: "none",
    color: "#f0e8d8", fontFamily: "inherit", fontSize: 15, lineHeight: 1.7,
    resize: "none", padding: 0,
  },
  sendBtn: {
    background: "radial-gradient(circle, rgba(200,160,80,0.4) 0%, rgba(139,90,200,0.3) 100%)",
    border: "1px solid rgba(200,160,80,0.5)", borderRadius: "50%",
    width: 40, height: 40, cursor: "pointer", color: "#d4a84b",
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @keyframes ringPulse {
    0%, 100% { transform: scale(1); opacity: 0.6; box-shadow: 0 0 0 0 rgba(200,160,80,0); }
    50% { transform: scale(1.08); opacity: 1; box-shadow: 0 0 20px 4px rgba(200,160,80,0.3); }
  }
  ...
  .send-btn:hover:not(:disabled) { background: radial-gradient(circle, rgba(200,160,80,0.6) 0%, rgba(139,90,200,0.5) 100%) !important; transform: scale(1.1); }
