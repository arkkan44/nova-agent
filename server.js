...
app.get("/api/directives", (req, res) => {
  res.json({ directives: loadDirectives() });
});
...
<body style="background:#050a0f; color:#e8d8b8; font-family:'Georgia', serif; padding:40px 0; margin:0;">
  <div style="max-width:560px; margin:0 auto; padding:0 24px;">
    ...
    <div style="text-align:center; margin-bottom:40px;">
      <a href="https://nova.coeurandco.com" style="display:inline-block; background:rgba(200,160,80,0.15); border:1px solid rgba(200,160,80,0.4); border-radius:30px; padding:12px 32px; color:#c8a050; text-decoration:none; font-size:13px; letter-spacing:2px;">Continuer votre voyage ✦</a>
    </div>
...
<!DOCTYPE html>
<html>
<body style="background:#050a0f; color:#e8d8b8; font-family:'Georgia', serif; padding:40px 24px; margin:0;">
  <div style="max-width:480px; margin:0 auto;">
    <div style="text-align:center; margin-bottom:32px;">
      <h1 style="font-size:28px; font-weight:400; letter-spacing:10px; color:#c8a050; margin:0;">NOVA</h1>
      <p style="font-size:11px; letter-spacing:3px; color:#706050; margin:8px 0 0; text-transform:uppercase;">Nouvel inscrit</p>
    </div>
    <div style="background:rgba(200,160,80,0.06); border:1px solid rgba(200,160,80,0.2); border-radius:16px; padding:28px;">
      <p style="font-size:13px; color:#a09080; margin:0 0 12px; letter-spacing:1px; text-transform:uppercase;">✦ Un nouvel être a rejoint NOVA</p>
      <p style="font-size:18px; color:#d4a84b; margin:0 0 20px;">${userEmail}</p>
      <p style="font-size:13px; color:#706050; margin:0;">Inscrit le ${dateStr} à ${timeStr}</p>
    </div>
    <div style="text-align:center; margin-top:28px;">
      <a href="https://nova.coeurandco.com/admin" style="display:inline-block; background:rgba(200,160,80,0.15); border:1px solid rgba(200,160,80,0.4); border-radius:30px; padding:10px 28px; color:#c8a050; text-decoration:none; font-size:12px; letter-spacing:2px;">Voir le bureau admin ✦</a>
    </div>
  </div>
</body>
</html>`
    ...
    console.error("Erreur webhook new-user:", e.message);
    res.status(500).json({ error: e.message });
  }
});
...
app.get("/api/admin/conversation-summary/:convId", requireAdmin, async (req, res) => {
  try {
    ...
    const transcript = msgs.map(m =>
      `${m.role === "user" ? "Utilisateur" : "NOVA"}: ${m.content}`
    ).join("\n");
